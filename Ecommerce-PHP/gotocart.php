...
<div class="categories">
  ...
  <div>
    ...
    <table class="" style="table-layout: fixed">
        ...
        $view_users_query="select * from cart where p_email='$user'";
        ...
        <tr>
            ...
            <td><a href="delete_item.php?del=<?php echo $p_id; ?>"><button><i class="fa fa-close"></i></button></a></td>
        </tr>

        <?php } ?>
        <tr>
        <th  colspan='3'style="background:white"><strong>Delivery Charge</strong></th>
        <th colspan='2'style="background:white"><strong>Rs.50</strong></th>
      </tr>
        <tr>

            <th colspan='3'style="background:white"><strong>Total</strong></th>
            <th colspan='2'style="background:grey"><strong>Rs.<?php echo $t_price;?></strong></th>
        </tr>
    </table>
            <b><h3>Total Price:<?php echo $t_price+50;?></h3></b>

    <br><form method="POST" action="gotocart.php">
    <h2><input type="submit" name="buynow" value="Buy Now" class="button-3"></form>
    </h2><div>
</div>
</fieldset>
</fieldset>
...
if(isset($_POST['buynow']))
{
    ...
    $update_status=("UPDATE users SET action='$user_status' where user_email = '$user_email' LIMIT 1");
    ...
    $check_user="select * from cart WHERE p_email='$user_email'";
    $run=mysqli_query($dbcon,$check_user);
    if(mysqli_num_rows($run))
    {
           echo "<script>window.open('thankyou.php','_self')</script>";
           $insert_user="insert into bill (email,status) VALUE ('$user_email','Ordered')";
           $ru=mysqli_query($dbcon,$insert_user);
    }
       else
    {
        echo"<script>alert('No item added!')</script>";
    }
    $delete_query="delete from cart_approval WHERE p_email='$user_email' and delivery_status='Delivered'";
